...
bool AnisotropicDiffusionFilter::filter(Vol3D<uint8> &vOut, const Vol3D<uint8> &vIn, int verbosity)
{
  ...
  for (int n = 0; n < nIterations; n++)
  {
    ...
    int index2 = vIn.index(2, 2, 1);                                // as seen here, where the indices are
    ...
    for (int i = 2; i < Imax; i++)
    {
      for (int j = 2; j < Jmax; j++)
      {
        ...
        Cb = C[(square(int(c0))
               +square(int(cptr[-zStride + yStride] - cptr[-zStride - yStride]))
               +square(int(cptr[-zStride + 1] - cptr[-zStride - 1])))];
        Out[index2++] = c0 + (char)((Ce * int(cptr[2] - c0)
                                    -Cw * int(c0 - cptr[-2])
    ...
    if (n != (nIterations - 1))
    {
        memcpy(In, Out, datasize + 2 * slicesize);
    }
  }
  delete[] In;
