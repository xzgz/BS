...
#define BIGZ 2.0	/* Must be greater than or equal to 1.0 */
...
AIR_Pixels ***AIR_r_scan_chirp(AIR_Pixels ***volume, const struct AIR_Key_info *stats, struct AIR_Key_info *new_stats, double **es, const double scale, AIR_Error *errcode)

{
	...
	}
	{
		...
		if(stats->z_dim>1) z_dim1=(unsigned int)ceil(BIGZ*stats->z_dim);
		else z_dim1=1;
		...
		/* Allocate memory */
		{
				...
				temp_plane1=AIR_matrix2f(x_dim3,y_dim1);
				{
					...
					wa2=AIR_matrix1(2*dim1max);
					{
						...
						new=AIR_matrix1(2*dim2max);
						{
							...
							if(maxmem<z_dim1+z_dim2*ceil(fabs(e22))) maxmem=z_dim1+z_dim2*ceil(fabs(e22));
			...
			if(!new_volume||!temp_volume1||!temp_plane1||!old||!wa2||!new||!ch||!work1||!work2||!work3||!wa){
				printf("%s: %d: ",__FILE__,__LINE__);
				printf("r_scan_chirp() memory allocation failure\n");
		...
		/*Prepass interpolate along z */
		{
			...
			double w=2.0*pi/(stats->z_dim*BIGZ);
			double a=0.0;
			unsigned int nfft=1;

			while(nfft<=stats->z_dim+z_dim1-1) nfft*=2;
			if(stats->z_dim>1 && BIGZ!=1.0){
				...
				for(j=0;j<stats->y_dim;j++){
					...
					for(i=0;i<stats->x_dim;i++){
						...
						/* Copy back */
						{
							...
							for(k=0;k<z_dim1;k++) temp_volume1[k][j][i]=(float)new[2*k];
						}
					}
				}
			}
			...
			for(k=0;k<z_dim1;k++,x_k+=e20,x_k2+=e20i,y_k+=e21){
				...
				if(stats->y_dim>1 && BIGY!=1.0){
					...
					for(j=0;j<stats->y_dim;j++){
						...
						for(i=0;i<stats->x_dim;i++){
							temp_plane1[j][i]=temp_volume1[k][j][i];
						}
					}
				}
					...
					while(nfft<=x_dim1+x_dim2*oversamp-1) nfft*=2;
					{
						...
						for(j=0;j<y_dim1;j++,x_j+=e10){
							...
							/* Interpolate */
							{
								...
								a=2.0*pi*(e30+k*e20+j*e10)/x_dim1;
							...
							/* Copy back */
							{
								...
								for(i=0,x_i=x_j;i<x_dim2;i++,x_i+=e00){
									if(x_i>=0 && x_i<=x_dim1-1) temp_plane1[j][i]=(float)new[2*i];
		...
		{
			unsigned int oversamp=(unsigned int)ceil(fabs(e22));
			double w=2.0*pi/(z_dim1*oversamp/e22);
			AIR_Boolean inited=FALSE;
			unsigned int ifac[20],ifac2[20];
			unsigned int nfft=1;
				...
				for(j=0;j<y_dim2;j++,z_j+=e12){
					...
					for(i=0;i<x_dim2;i++,z_i+=e02){
						...
						if(z_dim1>1){
							...
							double a=2.0*pi*(e32+i*e02+j*e12)/z_dim1;
						...
						/* Copy back with rounding */
						{
							...
							for(k=0;k<z_dim2;k++,z_k+=e22){
								if(z_k>=0 && z_k<=z_dim1-1){
									new[2*k]*=scale;
									if(new[2*k]<0.0) new_volume[k][j][i]=0;
									else{
										new[2*k]+=.5;
										if(new[2*k]>(double)AIR_CONFIG_MAX_POSS_VALUE) new_volume[k][j][i]=AIR_CONFIG_MAX_POSS_VALUE;
										else new_volume[k][j][i]=(AIR_Pixels)new[2*k];
									}
								}
