...
#define STRICTNESS 0	/*STRICTNESS=0 computes mean of all non-zero pixels at a given location*/
...
#define RESCALE FALSE	/*RESCALE=TRUE results in output being rescaled with maximum of MAX_POSS_VALUE*/
			/*RESCALE=FALSE will not rescale input data (largest scaling factor of inputs */
			/* will apply to output */

AIR_Error AIR_do_softmeannorescale(const char *program, const char *output, const unsigned int n, char **inputs, const char *suffix, const AIR_Boolean ow)
