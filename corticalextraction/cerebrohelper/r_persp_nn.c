...
/*
 * AIR_Pixels ***r_persp_nn()
 *
 * This routine will reslice a file based on the unit vectors es
 * It uses nearest neighbor interpolation and can do perspective
 *  transformations
 *
 * returns:
...
AIR_Pixels ***AIR_r_persp_nn(AIR_Pixels ***volume, const struct AIR_Key_info *stats, struct AIR_Key_info *new_stats, double **es, const double scale, AIR_Error *errcode)
						...
						if(x_p>=0.0 && x_p<=x_max1){
							...
							if(y_p>=0.0 && y_p<=y_max1){
								...
								if(z_p>=0.0 && z_p<=z_max1){
									...
									x_in=TRUE;
									{
										double total=(double)volume[z_up][y_up][x_up];
										total*=scale;
										total+=.5;
