...
static double sinc(const double alpha, const double pi)

{	
	if(alpha==0.0) return 1.0;
	return sin(pi*alpha)/(pi*alpha);
}
...
AIR_Pixels ***AIR_r_scan_sinc(AIR_Pixels ***volume, const struct AIR_Key_info *stats, struct AIR_Key_info *new_stats, double **es, const double scale, AIR_Error *errcode)

{
	...
	}
	{
		...
		if(stats->y_dim>1) y_dim1=(BIG*stats->y_dim)/SMALL;
		else y_dim1=1;
	
		if(stats->z_dim>1) z_dim1=(BIG*stats->z_dim)/SMALL;
		else z_dim1=1;
					...
					/* Interpolate */
					{
						...
						for(i=0;i<x_dim2;i++,i3++,x_i+=e00){
							if(x_i>=0.0 && x_i<=(x_dim1-1)){
								...
								for(i4=0;i4<x_dim1;i4++,i2++){
									total+=*i2*sinc(x_i-i4,pi);
								}
								*i3=(float)total;
							}
							else *i3=0.0F;
					...
					/* Interpolate */
					{
						...
						for(j=0;j<y_dim2;j++,j3++,x_j+=e10i,y_j+=e11){
							if(y_j>=0.0 && y_j<=(y_dim1-1) && x_j>=0.0 && x_j<=(x_dim1-1)){
								...
								for(j4=0;j4<y_dim1;j4++,j2++){
									total+=*j2*sinc(y_j-j4,pi);
								}
								*j3=(float)total;
							}
							else *j3=0.0F;
					...
					/* Interpolate */
					{
						...
						for(k=0;k<z_dim2;k++,k3++,z_k+=e22){
							if(z_k>=0.0 && z_k<=(z_dim1-1)){
								...
								for(k4=0;k4<z_dim1;k4++,k2++){
									total+=*k2*sinc(z_k-k4,pi);
								}
								*k3=(float)total;
							}
							else *k3=0.0F;
