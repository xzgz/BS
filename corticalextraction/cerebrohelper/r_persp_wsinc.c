...
AIR_Pixels ***AIR_r_persp_wsinc(AIR_Pixels ***volume, const struct AIR_Key_info *stats, struct AIR_Key_info *new_stats, double **es, const double scale, const unsigned int xkern, const unsigned int ykern, const unsigned int zkern,  AIR_Error *errcode)
						...
						if(x_p>=0.0 && x_p<=x_max1){
							...
							if(y_p>=0.0 && y_p<=y_max1){
								...
								if(z_p>=0.0 && z_p<=z_max1){
									...
									*i3=AIR_interp_wsinc_3D(stats, volume, x_p, y_p, z_p, scale, window, pi);			
								}
								else if(x_in) break;
							}
							else if(x_in) break;
