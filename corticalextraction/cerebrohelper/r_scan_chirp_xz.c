...
AIR_Pixels ***AIR_r_scan_chirp_xz(AIR_Pixels ***volume, const struct AIR_Key_info *stats, struct AIR_Key_info *new_stats, double **es, const double scale, AIR_Error *errcode)

{
	...
	}
	{
		...
		if(stats->z_dim>1) z_dim1=(unsigned int)ceil(BIGY*stats->z_dim);
		else z_dim1=1;	
		...
		/*Allocate memory*/
		{
			new_volume=AIR_create_vol3(x_dim2,y_dim2,z_dim2);
			{
				...
				unsigned int x_dim3, z_dim4;
	
				if(x_dim2>x_dim1) x_dim3=x_dim2;
				else x_dim3=x_dim1;
	
				if(z_dim2>stats->z_dim) z_dim4=z_dim2;
				else (z_dim4=stats->z_dim);
	
				temp_volume1=AIR_matrix3f(x_dim3,z_dim4,y_dim1);
	
				/* Need to have a plane that is larger of x_sizes by old z size */
				temp_plane1=AIR_matrix2f(x_dim3,z_dim1);
				{
					...
					wa2=AIR_matrix1(2*dim1max);
					{
						...
						if(z_dim1>dim2max) dim2max=z_dim1;	/* For prepass */

						new=AIR_matrix1(2*dim2max);
						{
							unsigned int maxmem=x_dim1+x_dim2*ceil(fabs(e00));
							if(maxmem<z_dim1+z_dim2*ceil(fabs(e11))) maxmem=z_dim1+z_dim2*ceil(fabs(e11));
							if(maxmem<stats->z_dim+z_dim1) maxmem=stats->z_dim+z_dim1;	/* For prepass */
			...
			if(!new_volume||!temp_volume1||!temp_plane1||!old||!wa2||!new||!ch||!work1||!work2||!work3||!wa){
				printf("%s: %d: ",__FILE__,__LINE__);
				printf("chirperxz() memory allocation failure\n");
		...
		/* Now, for each plane, we prepass interpolate along y and then interpolate x and y */
		{
			...
			for(k=0;k<y_dim1;k++,x_k+=e20,x_k2+=e20i,y_k+=e21){
				...
				/* Prepass interpolate along y*/
				{
					...
					double w=2.0*pi/(stats->z_dim*BIGY);
					double a=0.0;

					unsigned int nfft=1;

					while(nfft<=stats->z_dim+z_dim1-1) nfft*=2;
	
					if(stats->z_dim>1 && BIGY!=1.0){
						...
						for(i=0;i<stats->x_dim;i++){
							...
							/* Copy */
							{
								...
								for(j=0;j<stats->z_dim;j++){
									old[2*j]=(double)volume[j][k][i];
									old[2*j+1]=0.0;
								}
							}
							/* Interpolate */
							{
								AIR_chirpterp(stats->z_dim,old,z_dim1,new,nfft,work1,work2,work3,ch,wa2,wa,ifac2,ifac,w,a,oversamp,&inited);
							}
							/* Copy back */
							{
								unsigned int j;
	
								for(j=0;j<z_dim1;j++) temp_plane1[j][i]=(float)new[2*j];
							}
						}
					}
					else{
						...
						for(j=0;j<stats->z_dim;j++){
							...
							for(i=0;i<stats->x_dim;i++){
								temp_plane1[j][i]=(float)volume[j][k][i];
							}
						}
					}
				}
				...
				/* Interpolate from plane along y */
				{
					...
					double w=2.0*pi/(z_dim1*oversamp/e11);
					AIR_Boolean inited=FALSE;
					unsigned int ifac[20],ifac2[20];
					
					unsigned int nfft=1;
	
					while(nfft<=z_dim1+z_dim2*oversamp-1) nfft*=2;
					{
						double
							x_i=x_k2,
							y_i=y_k;
						...
						for(i=0;i<x_dim2;i++,x_i+=e00i,y_i+=e01){
							...
							if(z_dim1>1){
								...
								double a=2.0*pi*(e31+k*e21+i*e01)/z_dim1;
								AIR_chirpterp(z_dim1,old,z_dim2,new,nfft,work1,work2,work3,ch,wa2,wa,ifac2,ifac,w,a,oversamp,&inited);
							}
							else new[0]=old[0];
							...
							/* Copy back */
							{
								...
								for(j=0;j<z_dim2;j++,x_j+=e10i,y_j+=e11){
									if(y_j>=0 && y_j<=z_dim1-1 && x_j>=0 && x_j<=x_dim1-1){
		...
		/*Interpolate from float volume along z*/
		{
			...
			for(j=0;j<z_dim2;j++,z_j+=e12){
				...
				for(i=0;i<x_dim2;i++,z_i+=e02){
					...
					for(k=0;k<y_dim2;k++,z_k+=e22){
						if(z_k>=0 && z_k<=y_dim1-1){
							...
							new[0]*=scale;
							if(new[0]<0.0) new_volume[j][k][i]=0;
							else{
								new[0]+=.5;
								if(new[0]>(double)AIR_CONFIG_MAX_POSS_VALUE) new_volume[j][k][i]=AIR_CONFIG_MAX_POSS_VALUE;
								else new_volume[j][k][i]=(AIR_Pixels)new[0];
							}
						}
						else new_volume[j][k][i]=0;
					}
				}
			}
		}
