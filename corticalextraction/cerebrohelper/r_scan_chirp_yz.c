...
AIR_Pixels ***AIR_r_scan_chirp_yz(AIR_Pixels ***volume, const struct AIR_Key_info *stats, struct AIR_Key_info *new_stats, double **es, const double scale, AIR_Error *errcode)
	...
	}
	{
		...
		/*Allocate memory*/
		{
				...
				temp_plane1=AIR_matrix2f(z_dim3,y_dim1);
				{
					...
					wa2=AIR_matrix1(2*dim1max);
					{
						...
						new=AIR_matrix1(2*dim2max);
						{
							...
							unsigned int maxmem=z_dim1+z_dim2*ceil(fabs(e00));
			...
			if(!new_volume||!temp_volume1||!temp_plane1||!old||!wa2||!new||!ch||!work1||!work2||!work3||!wa){
				printf("%s: %d: ",__FILE__,__LINE__);
				printf("chirperyz() memory allocation failure: wa\n");
		...
		/* Now, for each plane, we prepass interpolate along y and then interpolate x and y */
		{
			...
			for(k=0;k<x_dim1;k++,x_k+=e20,x_k2+=e20i,y_k+=e21){
				...
				/* Prepass interpolate along y*/
				{
					...
					if(stats->y_dim>1 && BIGY!=1.0){
						...
						for(j=0;j<stats->y_dim;j++){
							...
							for(i=0;i<stats->z_dim;i++){
								temp_plane1[j][i]=(float)volume[i][j][k];
							}
						}
					}
				}
				...
				if(z_dim1>1){
					...
					double w=2.0*pi/(z_dim1*oversamp/e00);
					AIR_Boolean inited=FALSE;
					unsigned int ifac[20],ifac2[20];
					unsigned int nfft=1;
					...
					while(nfft<=z_dim1+z_dim2*oversamp-1) nfft*=2;
					{
						...
						for(j=0;j<y_dim1;j++,x_j+=e10){
							...
							/* Interpolate */
							{
								double a=2.0*pi*(e30+k*e20+j*e10)/z_dim1;
							...
							/* Copy back */
							{
								...
								for(i=0;i<z_dim2;i++,x_i+=e00){
									if(x_i>=0 && x_i<=z_dim1-1){
				...
				/* Interpolate from plane along y */
				{
					...
					while(nfft<=y_dim1+y_dim2*oversamp-1) nfft*=2;
					{
						...
						for(i=0;i<z_dim2;i++,x_i+=e00i,y_i+=e01){
							...
							/* Copy back */
							{
								...
								for(j=0;j<y_dim2;j++,x_j+=e10i,y_j+=e11){
									if(y_j>=0 && y_j<=y_dim1-1 && x_j>=0 && x_j<=z_dim1-1){
		...
		/*Interpolate from float volume along z*/
		{
			...
			for(j=0;j<y_dim2;j++,z_j+=e12){
				...
				for(i=0;i<z_dim2;i++,z_i+=e02){
	
					double z_k=z_i;
					unsigned int k;
	
					for(k=0;k<x_dim2;k++,z_k+=e22){
						if(z_k>=0 && z_k<=x_dim1-1){
							...
							new[0]*=scale;
							if(new[0]<0.0) new_volume[i][j][k]=0;
							else{
								new[0]+=.5;
								if(new[0]>(double)AIR_CONFIG_MAX_POSS_VALUE) new_volume[i][j][k]=AIR_CONFIG_MAX_POSS_VALUE;
								else new_volume[i][j][k]=(AIR_Pixels)new[0];
							}
						}
						else new_volume[i][j][k]=0;
					}
				}
			}
		}
