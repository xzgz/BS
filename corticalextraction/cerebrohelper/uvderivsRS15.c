		...
		for (r=0, rr=**pixel5; r<r_term; r+=samplefactor, rr+=samplefactor){
		...
		
			{
				...
				{				
					...
					double expix4=((n7+n1-n5-n3)*a+(n6+n0-n4-n2)*d);
					double eypix4=((n7+n2-n6-n3)*b+(n5+n0-n4-n1)*e);
					double ezpix4=((n7+n4-n6-n5)*c+(n3+n0-n2-n1)*f);
					...
					/*First derivatives*/
					{
						...
						{	
							unsigned int t;	
							{
								...
								for(t=BLOCK3; t<BLOCK4; t++, dcft3++, dxt++, dyt++, dzt++){
								
									/* dcf[t]+=2.0*(pix4-pix3*scale)*(dxpix4*dx[t]+dypix4*dy[t]+dzpix4*dz[t];*/
									*dcft3+=2.0*(pix4-spix3)*(dxpix4**dxt+dypix4**dyt+dzpix4**dzt);
								}
							}
							{
								for(; t<BLOCK5; t++, dcft3++){
							...
							}
							{
								...
								for(;t<BLOCK3;t++){
									...
									for(s=BLOCK0; s<BLOCK1; s++, ecfts++, dztdxs++){
										...
										/* ecf[t][s]+=2.0*dz[t]*dx[s]*(dxpix4*dzpix4+(pix4-pix3*scale)*eypix4);*/
										*ecfts+=*dztdxs*(dxpix4*dzpix4+(pix4-spix3)*eypix4);
									}
	
									for(; s<BLOCK2; s++, ecfts++, dztdys++){
									
										/* ecf[t][s]+=2.0*dz[t]*dy[s]*(dypix4*dzpix4+(pix4-pix3*scale)*expix4);*/
										*ecfts+=*dztdys*(dypix4*dzpix4+(pix4-spix3)*expix4);
									}
								...
								for(; t<BLOCK4; t++, dxt++, dyt++, dzt++){
									...
									
									{
										...
										for(s=BLOCK0; s<BLOCK1; s++, ecfts++, dxs++, exts++){
											...
											/*ecf[t][s]+=2.0*dxpix4*dx[s]*(dxpix4*dx[t]+dypix4*dy[t]+dzpix4*dz[t]);*/
											*ecfts+=2.0*dxpix4**dxs*(dxpix4**dxt+dypix4**dyt+dzpix4**dzt);
	
											/* ecf[t][s]+=2.0*(pix4-pix3*scale)*(dxpix4*ex[t][s]); */
											*ecfts+=2.0*(pix4-spix3)*(dxpix4**exts);
											
											/* ecf[t][s]+=2.0*(pix4-pix3*scale)*dx[s]*(ezpix4*dy[t]+eypix4*dz[t]);*/
											*ecfts+=2.0*(pix4-spix3)**dxs*(ezpix4**dyt+eypix4**dzt);
										}
									}
									{
										double *dys=dxyz;
										...
										for(; s<BLOCK2; s++, ecfts++, dys++, eyts++){
											...
											/* ecf[t][s]+=2.0*dypix4*dy[s]*(dxpix4*dx[t]+dypix4*dy[t]+dzpix4*dz[t]);*/
											*ecfts+=2.0*dypix4**dys*(dxpix4**dxt+dypix4**dyt+dzpix4**dzt);
											
											/* ecf[t][s]+=2.0*(pix4-pix3*scale)*(dypix4*ey[t][s]); */
											*ecfts+=2.0*(pix4-spix3)*(dypix4**eyts);
											
											/* ecf[t][s]+=2.0*(pix4-pix3*scale)*dy[s]*(ezpix4*dx[t]+expix4*dz[t]); */
											*ecfts+=2.0*(pix4-spix3)**dys*(ezpix4**dxt+expix4**dzt);
										}
									}
									{
										double *dzs=dxyz;
										
										for(; s<BLOCK3; s++, ecfts++, dzs++, ezts++){
										
											/* ecf[t][s]+=2.0*dzpix4*dz[s]*(dxpix4*dx[t]+dypix4*dy[t]+dzpix4*dz[t]); */
											*ecfts+=2.0*dzpix4**dzs*(dxpix4**dxt+dypix4**dyt+dzpix4**dzt);
											
											/* ecf[t][s]+=2.0*(pix4-pix3*scale)*(dzpix4*ez[t][s]);*/
											*ecfts+=2.0*(pix4-spix3)*(dzpix4**ezts);
											
											/* ecf[t][s]+=2.0*(pix4-pix3*scale)*dz[s]*(eypix4*dx[t]+expix4*dy[t]); */
											*ecfts+=2.0*(pix4-spix3)**dzs*(eypix4**dxt+expix4**dyt);
										}
									}
									{
										double *dxs=dx;
										...
										for(; s<=t; s++, ecfts++, dxs++, dys++, dzs++, exts2++, eyts2++, ezts2++){
										
											/* ecf[t][s]+=2.0*(dxpix4*dx[s]+dypix4*dy[s]+dzpix4*dz[s])*(dxpix4*dx[t]+dypix4*dy[t]+dzpix4*dz[t]);*/
											*ecfts+=2.0*(dxpix4**dxs+dypix4**dys+dzpix4**dzs)*(dxpix4**dxt+dypix4**dyt+dzpix4**dzt);
											
											/* ecf[t][s]+=2.0*(pix4-pix3*scale)*(dxpix4*ex[t][s]+dypix4*ey[t][s]+dzpix4*ez[t][s]); */
											*ecfts+=2.0*(pix4-spix3)*(dxpix4**exts2+dypix4**eyts2+dzpix4**ezts2);
	...
	
							{
								for(; t<BLOCK5; t++){
									...
									}
									{
										...
										for(; s<BLOCK4; s++, ecfts++, dxs++, dys++, dzs++){
										
											/* ecf[t][s]+=-2.0*pix3*(dxpix4*dx[s]+dypix4*dy[s]+dzpix4*dz[s]); */
											*ecfts+=-2.0*pix3*(dxpix4**dxs+dypix4**dys+dzpix4**dzs);
										}
									}
									{
										for(; s<=t; s++, ecfts++){
