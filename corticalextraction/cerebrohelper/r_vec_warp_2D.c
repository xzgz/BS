...
AIR_Pixels ***AIR_r_vec_warp_2D(AIR_Pixels ***volume, const struct AIR_Key_info *stats, const struct AIR_Key_info *new_stats, float ***field, double **es, const double scale, const unsigned int order, const unsigned int *window, AIR_Pixels (*interp2D)(const struct AIR_Key_info *, AIR_Pixels ***, const double, const double, const unsigned int, const double, const unsigned int *, const double), AIR_Error *errcode)
				...
				for (;k<z_dim2;k++,pk3++){
					...
					for (;j<y_dim2;j++,pj3++,fj++){
						...
						for (;i<x_dim2;i++,pi3++,fi++){
							...
							double x=*fd++;
							double y=*fd;       /* Final reference to fd */
		
							double x_p, y_p;
		
							if(AIR_warp2D(es,x,y,&x_p,&y_p,order,dx)!=0){
								continue;
							}
