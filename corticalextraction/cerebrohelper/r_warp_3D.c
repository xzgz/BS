...
AIR_Pixels ***AIR_r_warp_3D(AIR_Pixels ***volume, const struct AIR_Key_info *stats, const struct AIR_Key_info *new_stats, double **es, const double scale, const unsigned int order, const unsigned int *window, AIR_Pixels (*interp3D)(const struct AIR_Key_info *, AIR_Pixels ***, const double, const double, const double, const double, const unsigned int *, const double), AIR_Error *errcode)
				...
				for (; k<z_dim2; k++, pk3++){
					...
					for (; j<y_dim2; j++, pj3++){
						...
						for (; i<x_dim2; i++, pi3++){
							...
							if(AIR_warp3D(es,(double)i,(double)j,(double)k,&x_p,&y_p,&z_p,order,dx)!=0){
								continue;
							}
