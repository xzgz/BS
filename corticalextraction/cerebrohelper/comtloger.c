...
/*
 * Averages a set of transformations to compute the mean, original transformations are unchanged
 * 
 * Routine iterates until absolute values of all means are less than TINY, then
 *  iterates until failure to improve further.
 *
 * Storage4 needs to be allocated matrix3(>=N,>=N,>=7)
 *
 * Returns:
 *	0 if successful
...
AIR_Error AIR_comtloger(const unsigned int n, double ***en, const unsigned int count, double **mean, double ***storage4, AIR_Boolean is_affine)
